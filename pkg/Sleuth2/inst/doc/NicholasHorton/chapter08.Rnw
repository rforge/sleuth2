...
Next we can observe the relationship between log({\tt Time}) and {\tt Voltage}, the following figure is akin to Display 8.4 (page 210).
...
For the interpretation of the model we first exponentiat the estimated coefficients since the response variable is logged as shown on page 215.
...
Next we can check for linearity, the following figure is akin to the right side graph in Display 8.14 (page 225).
